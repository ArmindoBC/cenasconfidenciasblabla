...
.factory('security', ['$http', '$q', '$location', 'securityRetryQueue', '$modal', '$sessionStorage', function($http, $q, $location, queue, $modal, $sessionStorage) {
  ...
  var service = {
    ...
    socialLogin: function(provider, code){
      ...
      var promise = $http.get(url).then(function(res){
        ...
        if (data.success) {
          ...
          $sessionStorage.sessionToken = data.userSession.token;
    ...
    // Attempt to authenticate a user by Active Directory credentials
    loginAD: function(username, password) {
      var request = $http.post('/api/login-ad', {
        username: username,
        password: password
      });
      return request.then(function(response) {
    ...
    logout: function(redirectTo) {
      $http({
          method:"POST",
          url:'/api/logout',
          headers: {Authorization: $sessionStorage.sessionToken && 'Bearer ' + $sessionStorage.sessionToken }
      }).then(function() {
    ...
    isAdmin: function() { 
      return !!(service.currentUser && service.currentUser.roles && service.currentUser.roles.admin);
    }
  };
