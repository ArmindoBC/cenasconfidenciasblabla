...
class LogClient {
    ...
    Log(logData) {
      logData = logData == undefined ? {} : logData;
        ...
        if ( this.CheckLog(logMessage) && this.LogLevelAllowed(logMessage.level) ) {
            return this.Connect()
                .then(() => {
                    this.logChannel.assertQueue(rabbitConfigs.queueName, {durable: false});
     ...
     */
    CheckLog(logData) {
        if(!logData.level || typeof logData.level != 'string') return false;
        if(!logData.timestamp || typeof logData.timestamp != 'string') return false;
        if(!logData.machineName || typeof logData.machineName != 'string') return false;
        if(!logData.projectName || typeof logData.projectName != 'string') return false;
        if(!logData.category || typeof logData.category != 'string') return false;
        if(!logData.message || typeof logData.message != 'string') return false;
        return true;
